...
class Data extends AbstractData
{
     ...
     * @return string
     */
    public function getAllOptions($producttype)
    {
        $sliderOptions = '';

        $modulemethod="slider_design";
         if($producttype=="featured"){
             $modulemethod="featured_products";
         }else if($producttype=="new"){
             $modulemethod="new_products";

         }else if($producttype=="brand"){
             $modulemethod="brand_slider";
         }
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/vgslider.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info($producttype);
        $logger->info($modulemethod);

        $allConfig     = $this->getModuleConfig($modulemethod);
        foreach ($allConfig as $key => $value) {
            if ($key === 'item_slider') {
                $sliderOptions .= $this->getResponseValue($producttype);
     ...
     */
    public function getResponseValue($producttype)
    {
        $responsiveOptions = '';
         $responsiveConfig  = $this->getModuleConfig('slider_design/responsive')
            ...
            : [];
        if($producttype=="featured"){
            $responsiveConfig  = $this->getModuleConfig('featured_products/responsive')
            ? $this->unserialize($this->getModuleConfig('featured_products/item_slider'))
            : [];
        }else if($producttype=="new"){
            $responsiveConfig  = $this->getModuleConfig('new_products/responsive')
            ? $this->unserialize($this->getModuleConfig('new_products/item_slider'))
            : [];
        }else if($producttype=="brand"){
             $responsiveConfig  = $this->getModuleConfig('brand_slider/responsive')
            ? $this->unserialize($this->getModuleConfig('brand_slider/item_slider'))
            : [];
        }
